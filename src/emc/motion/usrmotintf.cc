...
void printEmcPose(EmcPose *pose) {
    printf("x=%f\ty=%f\tz=%f\tu=%f\tv=%f\tw=%f\ta=%f\tb=%f\tc=%f", pose->tran.x, pose->tran.y, pose->tran.z, pose->u,
           pose->v, pose->w, pose->a, pose->b, pose->c);
}
...
void usrmotPrintEmcmotDebug(emcmot_debug_t *d, int which) {
    ...
    case 0:
        ...
        printf("teleop desiredVel: \t%f\t%f\t%f\t%f\t%f\t%f\n", d->teleop_data.desiredVel.tran.x,
               d->teleop_data.desiredVel.tran.y, d->teleop_data.desiredVel.tran.z, d->teleop_data.desiredVel.a,
               d->teleop_data.desiredVel.b, d->teleop_data.desiredVel.c);
        printf("teleop currentVel: \t%f\t%f\t%f\t%f\t%f\t%f\n", d->teleop_data.currentVel.tran.x,
               d->teleop_data.currentVel.tran.y, d->teleop_data.currentVel.tran.z, d->teleop_data.currentVel.a,
        ...
        printf("teleop desiredAccel: \t%f\t%f\t%f\t%f\t%f\t%f\n", d->teleop_data.desiredAccel.tran.x,
               d->teleop_data.desiredAccel.tran.y, d->teleop_data.desiredAccel.tran.z, d->teleop_data.desiredAccel.a,
        ...
        printf("teleop currentAccel: \t%f\t%f\t%f\t%f\t%f\t%f\n", d->teleop_data.currentAccel.tran.x,
               d->teleop_data.currentAccel.tran.y, d->teleop_data.currentAccel.tran.z, d->teleop_data.currentAccel.a,
...
/*! \todo Another #if 0 */
    ...
    printf("\n");
        ...
        printf("cmd pos:      \t%f\t%f\t%f\t%f\t%f\t%f\n", s->carte_pos_cmd.tran.x, s->carte_pos_cmd.tran.y,
               s->carte_pos_cmd.tran.z, s->carte_pos_cmd.a, s->carte_pos_cmd.b, s->carte_pos_cmd.c);
        printf("act pos:      \t%f\t%f\t%f\t%f\t%f\t%f\n", s->carte_pos_fb.tran.x, s->carte_pos_fb.tran.y,
               s->carte_pos_fb.tran.z, s->carte_pos_fb.a, s->carte_pos_fb.b, s->carte_pos_fb.c);
        printf("joint data:\n");
/*! \todo Another #if 0 */
#if 0 /*! \todo FIXME - change to work with joint                                                                      \
         structures */
...
int usrmotInit(const char *modname) {
    ...
    if (retval < 0) {
        rtapi_print_msg(RTAPI_MSG_ERR, "usrmotintf: ERROR: could not access shared memory\n");
        rtapi_exit(module_id);
        return -1;
    }
    /* got it */
