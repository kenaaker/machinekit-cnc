...
struct TaskWrap : public Task, public bp::wrapper<Task> {
    ...
    int emcToolSetOffset(int pocket, int toolno, EmcPose offset, double diameter, double frontangle, double backangle,
                         int orientation) {
...
static const char *get_file(EMC_TASK_STAT &t) {
    return t.file;
}
static const char *get_command(EMC_TASK_STAT &t) {
    return t.command;
}
...
BOOST_PYTHON_MODULE(emctask) {
    ...
    scope().attr("__doc__") = "Task introspection\n";
    ...
    def("operator_text", operator_text,
        operator_text_overloads(args("id"), "send a informational messsage to the operator screen"));
    def("operator_display", operator_display,
        operator_display_overloads(args("id"), "send a messsage to the operator display"));

#define VAL(X) .value(#X, X)

    enum_<RCS_STATUS>("RCS_STATUS") VAL(RCS_EXEC) VAL(RCS_DONE) VAL(RCS_ERROR);

    enum_<EMC_TASK_MODE_ENUM>("EMC_TASK_MODE") VAL(EMC_TASK_MODE_MANUAL) VAL(EMC_TASK_MODE_AUTO) VAL(EMC_TASK_MODE_MDI);

    enum_<EMC_TASK_STATE_ENUM>("EMC_TASK_STATE") VAL(EMC_TASK_STATE_ESTOP) VAL(EMC_TASK_STATE_ESTOP_RESET)
        VAL(EMC_TASK_STATE_OFF) VAL(EMC_TASK_STATE_ON);

    enum_<EMC_TASK_EXEC_ENUM>("EMC_TASK_EXEC") VAL(EMC_TASK_EXEC_ERROR) VAL(EMC_TASK_EXEC_DONE)
        VAL(EMC_TASK_EXEC_WAITING_FOR_MOTION) VAL(EMC_TASK_EXEC_WAITING_FOR_MOTION_QUEUE)
            VAL(EMC_TASK_EXEC_WAITING_FOR_IO) VAL(EMC_TASK_EXEC_WAITING_FOR_MOTION_AND_IO)
                VAL(EMC_TASK_EXEC_WAITING_FOR_DELAY) VAL(EMC_TASK_EXEC_WAITING_FOR_SYSTEM_CMD);

    enum_<EMC_TASK_INTERP_ENUM>("EMC_TASK_INTERP") VAL(EMC_TASK_INTERP_IDLE) VAL(EMC_TASK_INTERP_READING)
        VAL(EMC_TASK_INTERP_PAUSED) VAL(EMC_TASK_INTERP_WAITING);

    enum_<EMC_IO_ABORT_REASON_ENUM>("EMC_IO_ABORT_REASON") VAL(EMC_ABORT_TASK_EXEC_ERROR) VAL(EMC_ABORT_AUX_ESTOP)
        VAL(EMC_ABORT_MOTION_OR_IO_RCS_ERROR) VAL(EMC_ABORT_TASK_STATE_OFF) VAL(EMC_ABORT_TASK_STATE_ESTOP_RESET)
            VAL(EMC_ABORT_TASK_STATE_ESTOP) VAL(EMC_ABORT_TASK_STATE_NOT_ON) VAL(EMC_ABORT_TASK_ABORT)
                VAL(EMC_ABORT_USER);
    ...
    class_<EMC_MOTION_STAT, noncopyable>("EMC_MOTION_STAT", no_init)
        ...
        .add_property("axis", bp::make_function(axis_w(&axis_wrapper), bp::with_custodian_and_ward_postcall<0, 1>()))

        .def_readwrite("spindle", &emcStatus->motion.spindle)
        .add_property("synch_di",
                      bp::make_function(synch_dio_w(&synch_di_wrapper), bp::with_custodian_and_ward_postcall<0, 1>()))
        .add_property("synch_do",
                      bp::make_function(synch_dio_w(&synch_do_wrapper), bp::with_custodian_and_ward_postcall<0, 1>()))
        .add_property("analog_input", bp::make_function(analog_io_w(&analog_input_wrapper),
                                                        bp::with_custodian_and_ward_postcall<0, 1>()))
        .add_property("analog_output", bp::make_function(analog_io_w(&analog_output_wrapper),
                                                         bp::with_custodian_and_ward_postcall<0, 1>()));
    ...
    class_<EMC_TASK_STAT, noncopyable>("EMC_TASK_STAT", no_init)
        ...
        .add_property("activeGCodes", bp::make_function(active_g_codes_tw(&activeGCodes_wrapper),
                                                        bp::with_custodian_and_ward_postcall<0, 1>()))
        .add_property("activeMCodes", bp::make_function(active_m_codes_tw(&activeMCodes_wrapper),
                                                        bp::with_custodian_and_ward_postcall<0, 1>()))
        .add_property("activeSettings", bp::make_function(active_settings_tw(&activeSettings_wrapper),
                                                          bp::with_custodian_and_ward_postcall<0, 1>()))
    ...
    class_<EMC_TOOL_STAT, noncopyable>("EMC_TOOL_STAT", no_init)
        ...
        .add_property("toolTable",
                      bp::make_function(tool_w(&tool_wrapper), bp::with_custodian_and_ward_postcall<0, 1>()));

    class_<EMC_AUX_STAT, noncopyable>("EMC_AUX_STAT", no_init).def_readwrite("estop", &EMC_AUX_STAT::estop);
    ...
    pp::register_array_1<EMC_AXIS_STAT, EMC_AXIS_MAX, bp::return_internal_reference<1, bp::default_call_policies>>(
        "AxisArray");
}
